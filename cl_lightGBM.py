...
logging.basicConfig(filename='lightgbm.log', level=logging.INFO, 
...
logging.info("LightGBM (базовые параметры):")
...
# Настроенная модель LightGBM (если требуется)
# Пример настроенной модели (можно изменить параметры)
...
y_pred_tuned = lgb_model_tuned.predict(X_test)
...
logging.info("\nLightGBM (настроенные параметры):")
...
cv_scores_tuned = cross_val_score(lgb_model_tuned, X, y, cv=5, scoring='f1')
